...
import { Wand2, ThumbsUp, ThumbsDown, Lightbulb } from "lucide-react";
    ...
    <div className="space-y-6 pt-4">
        ...
        <Card className="border-primary/50">
          <CardHeader className="flex flex-row items-center gap-2 pb-2">
            ...
            <CardTitle className="text-lg font-headline text-primary">Strategic Advice</CardTitle>
          </CardHeader>
          <CardContent>
            <ul className="list-disc space-y-2 pl-5 text-sm text-muted-foreground">
              {analysis.strategicAdvice.map((point, index) => (
                <li key={`advice-${index}`}>{point}</li>
              ))}
              {analysis.strategicAdvice.length === 0 && <li>No specific strategic advice generated.</li>}
            </ul>
          </CardContent>
        </Card>
    </div>
